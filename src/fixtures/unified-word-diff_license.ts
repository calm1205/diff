export const unifiedWordDiffLicense = `diff --git a/src/fixtures/original.license.txt b/src/fixtures/original.license.txt
index 98f550c..5487db3 100644
--- a/src/fixtures/original.license.txt
+++ b/src/fixtures/original.license.txt
@@ -1,69 +1,69 @@
 商標及び著作物使用許諾契約書
[-Aテクノロジー-]{+MNTSQ+}株式会社（ 以下、「甲」という。） と、[-B商事-]{+テクノロジー+}株式会社（以下、「乙」という。）とは、甲が権利を有する「○○○○」に関わる商標と「○○○○」に関わる著作物とに関する使用許諾契約（以下「本契約」という）を、次のとおり締結する。
...
２．[-乙-]{+オツ+}は、第三者が本件商標及び本件著作物に関する甲の権利を侵害していることを発見した場合、直ちに、その内容を甲に連絡し、甲乙協力してその侵害排除に努める。
３．{+オツは、本契約の有効期間中、本件商標及び本件著作物を使用し+}乙{+が現実に販売した本件商品の製品名及び本件商標及び本件著作物の使用状況を記録し、各年度末から６０日以内に、甲に各年度についての使用状況の記録を提出し報告しなければならない。+}
{+４．オツ+}は、本件商標及び本件著作物の使用に関し、乙の責めに帰すべき理由で第三者から甲に対しなされた損害賠償その他の一切の請求に対して、乙は、自己の費用と責任においてこれを解決する。
